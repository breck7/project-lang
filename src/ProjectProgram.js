...
class ProjectProgram extends TreeProgram {
  getGrammarString() {
    return fs.readFileSync(this.getGrammarFilePath(), "utf8")
  }

  getGrammarFilePath() {
    return __dirname + "/project.grammar"
  }
  ...
  static getProjectProgram(arrayOfScriptPaths) {
    const files = new TreeProgram(arrayOfScriptPaths.join("\n"))
    const required = new TreeProgram()
